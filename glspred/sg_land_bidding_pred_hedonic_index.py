...
from sklearn.metrics import mean_squared_error as mse_, mean_absolute_percentage_error as mape
import logging
import SQL_connect
...
def hedonic(transactions, y_col, m_or_qtr, index_len):
    ...
    if len(hedonic_index) < index_len:
        ...
        mse = mse_(y, fit.predict(x)) * HEDONIC_PENALTY
...
if __name__ == "__main__":
    ...
    gls_raw = dbconn.read_data('''select * from data_science_test.sg_gls_bidding_master_filled_features''')
    gls = gls_raw[gls_raw.region == 'central region']
    ...
    hi_rebase.insert(loc=0, column="mean_price_psm_gfa",
                     value=gls[[y_col, "year_launch"]].groupby(by="year_launch").mean().values)
